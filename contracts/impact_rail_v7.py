...
def observe(sealed: dict) -> dict:
    ...
    try:
        ...
        metadata_response = gl.nondet.web.get(NPM_REGISTRY + "/" + package_path + "/" + quote(sealed["npm_version"], safe=".-_"), headers={"Accept": "application/json"})
        downloads_response = gl.nondet.web.get(NPM_DOWNLOADS + "/" + sealed["period_start"] + ":" + sealed["period_end"] + "/" + package_path, headers={"Accept": "application/json"})
        if metadata_response.status != 200 or downloads_response.status != 200:
            return dict(obs, reason="NPM_SOURCE_UNAVAILABLE")
        if not isinstance(metadata_response.body, bytes) or not isinstance(downloads_response.body, bytes) or len(metadata_response.body) > 48000 or len(downloads_response.body) > 48000:
            return dict(obs, reason="NPM_SOURCE_SIZE_LIMIT")
        metadata = json.loads(metadata_response.body.decode("utf-8"), object_pairs_hook=unique_json)
        downloads = json.loads(downloads_response.body.decode("utf-8"), object_pairs_hook=unique_json)
        if not isinstance(metadata, dict) or not isinstance(downloads, dict):
            return dict(obs, reason="NPM_SOURCE_INVALID")
        expected_downloads = downloads.get("downloads")
        ...
        adoption_ok = (downloads.get("package") == sealed["npm_package"] and downloads.get("start") == sealed["period_start"] and
                       downloads.get("end") == sealed["period_end"] and type(expected_downloads) is int and
                       0 <= expected_downloads < 2**63 and expected_downloads >= sealed["minimum_downloads"])
        ...
        if not adoption_ok:
            return dict(obs, reason="OBJECTIVE_ADOPTION_BELOW_THRESHOLD")
        ...
        obs["github_digest"] = digest({"repo": expected_repo, "target": sealed["target_commit"], "npm_package": sealed["npm_package"],
                                       ...
                                       "downloads": expected_downloads, "period": [sealed["period_start"], sealed["period_end"]]})
...
class ImpactRail(gl.Contract):
    ...
    def _build_terms(self, beneficiary: str, amount_wei: u256, github_owner: str, github_repository: str, base_commit: str, target_commit: str,
                     ...
                     npm_package: str, npm_version: str, period_start: str, period_end: str, minimum_downloads: u256) -> dict:
        ...
        start, end = self._date(period_start), self._date(period_end)
        require(start > 0 and end >= start and end - start <= 31 * 86400 and end + 86400 <= now(), "INVALID_ADOPTION_PERIOD")
        require(1 <= minimum_downloads <= MAX_DOWNLOAD_THRESHOLD, "UNSUPPORTED_DOWNLOAD_THRESHOLD")
        return {"version": VERSION, "contract": address_text(gl.message.contract_address), "profile": self.profile, "beneficiary": beneficiary.lower(), "amount_wei": str(amount_wei),
                ...
                "npm_package": npm_package, "npm_version": npm_version, "period_start": period_start, "period_end": period_end,
                "minimum_downloads": int(minimum_downloads)}
    ...
    def create_grant(self, beneficiary: str, amount_wei: u256, github_owner: str, github_repository: str, base_commit: str, target_commit: str,
                      artifact_path: str, artifact_sha256: str, release_tag: str, milestone_statement: str,
                     minimum_commits: u256, minimum_contributors: u256, coverage_start: u256, duration: u256, partial_payout_bps: u256,
                     npm_package: str, npm_version: str, period_start: str, period_end: str, minimum_downloads: u256) -> u256:
        ...
        terms = self._build_terms(beneficiary, amount_wei, github_owner, github_repository, base_commit, target_commit, artifact_path, artifact_sha256,
                                   ...
                                   npm_package, npm_version, period_start, period_end, minimum_downloads)
    ...
    def get_config(self) -> dict:
        return {"version": VERSION, "profile": self.profile, "testnet_window_seconds": "120-900", "max_verifiable_commits": MAX_VERIFIABLE_COMMITS,
                "max_download_threshold": MAX_DOWNLOAD_THRESHOLD, "max_attempts": MAX_ATTEMPTS,
                "sources": ["github-api", "github-raw-at-commit", "github-release", "npm-registry", "npm-downloads-api"],
                ...
                "payout_policy": "objective npm adoption mandatory; VERIFIED=100%; PARTIAL=sealed bps; REJECTED/EXPIRED=sponsor refund"}
